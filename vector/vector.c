...
struct vector* createVector(unsigned int length)
{
  ...
  v->datum = malloc(sizeof(void *) * length);
...
bool addToVector(struct vector *v, void *data) {
  void **newDatum;

  if (v->used == v->length) {
    newDatum = malloc(sizeof(void *) * v->length * 2);
...
bool insertIntoVector(struct vector *v, void *data, unsigned int index) {
...
void *elementInVector(struct vector *v, unsigned int index) {
  return v->datum[index];
}

// Does not resize vector
void *deleteFromVector(struct vector *v, unsigned int index) {
  void *val = elementInVector(v, index);
